...
test('Contact form submits successfully via Server Action', async ({ page }) => {
  ...
  // Our CustomModal or status message should show success (or failure if API credentials are not present in the CI environment)
  const resultMessage = page.locator('text=Message sent successfully!')
    .or(page.locator('text=Your message has been saved successfully!'))
    .or(page.locator('text=Failed to send message. Please try again later.'));
